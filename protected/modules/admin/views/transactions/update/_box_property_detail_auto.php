<?php if( ProTransactions::IsTenancyTransaction($mTransactions) ):?>
    <?php include "_box_property_type.php";?>
<?php endif; // end if(isset($_GET['add_property']) ?>

<div class="in-row clearfix ad_fix_radio_p_detail hide">
...
<script>
    ...
    function fnBuildRowItem(event, ui){
        ...
        ?>
        var requestUri = '<?php echo Yii::app()->createAbsoluteUrl($mapControllerAction,array('add_property'=>$add_property,'id'=>$id, 'type'=>$type, 'user_id'=> $mTransactions->user_id, 'change_listing'=>1));?>/listing_id/'+ui.item.id;

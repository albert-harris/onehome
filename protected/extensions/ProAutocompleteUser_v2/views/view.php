...
<script>
    $(function(){
        ...
        <?php if(($data['model']->$data['name_relation_user']) && isset($data['TriggerReadonlyInput']) ) :?>
            var remove_div = '<span class=\'remove_row_item\' onclick=\'fnRemoveNamev2(this, \"<?php echo $idField;?>\", \"<?php echo $idFieldCustomerID;?>\")\'></span>';
            $('<?php echo $idField;?>').val('<?php echo $info_name;?>').attr('readonly',true).after(remove_div);
        <?php endif;?>
    });
    
    function fnRemoveNamev2(this_, idField, idFieldCustomer){        
    ...
    function fnCallSomeFunctionv2(item, idField){
        <?php if(isset($data['CallFunctionTier'])):?>

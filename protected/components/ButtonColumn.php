...
class ButtonColumn extends CButtonColumn
{
    ...
    public function renderDataCellContent($row, $data)
    {
        ...
        foreach($this->buttons as $id=>$button)
        {
            ...
            if($this->evaluateID and isset($button['options']['data-email']))
            {
                $button['options']['data-email'] = $this->evaluateExpression($button['options']['data-email'], array('row'=>$row,'data'=>$data));
            }
            if($this->evaluateID and isset($button['options']['data-subject']))
            {
                $button['options']['data-subject'] = $this->evaluateExpression($button['options']['data-subject'], array('row'=>$row,'data'=>$data));
            }

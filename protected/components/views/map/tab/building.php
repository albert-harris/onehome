...
//		var myPoint = new mxn.LatLonPoint(1.315074,103.842537);
                ...
                <?php if(isset($dataJson[1093]) && is_array($dataJson[1093]) && count($dataJson[1093])>0): foreach ($dataJson[1093] as $k=>$v):   ?>
					<?php
					$a = isset($v['a']) ? $v['a'] : '';
					?>
                        m.addMarkerWithData(new mxn.Marker(new mxn.LatLonPoint(<?php echo $v['y'] ?>, <?php echo $v['x'] ?>)) ,{
                            label : "<?php echo $v['v'] ?>",
                            infoBubble : "<div class='map_test'><div class='map_title_box_inner'><b><?php echo $v['v']; ?></b><br><?php echo $a; ?> </div><div style='width:100px;'><img src='<?php echo Yii::app()->theme->baseUrl; ?>/img/infobubble1.png' class='map_verz_chi_my'><img src='<?php echo Yii::app()->theme->baseUrl; ?>/img/info-close.png' style='position:absolute;right:12px;top:0px;'></div></div>",
                            marker : 8,
                ...
                m.addMarkerWithData(new mxn.Marker(myPoint) ,{
                        ...
                        infoBubble: "<div class='map_test'><div class='map_title_box_inner'><b><?php echo $data['title'] ?></b> </div><div style='width:100px;'><img src='<?php echo Yii::app()->theme->baseUrl; ?>/img/infobubble1.png' class='map_verz_chi_my'><img src='<?php echo Yii::app()->theme->baseUrl; ?>/img/info-close.png' style='position:absolute;right:12px;top:0px;'></div></div>",
                        hover : true
                });
        });
</script>
...
<div class="row">
	...
	<div class="col-md-3">
		...
		<div class="map-item" >
           <div class="box-3">
               <h3 class="title-2">Nearest Building</h3>
				<ul class="list-2 list-unstyled" style="max-height: 360px;overflow-y: scroll;overflow: auto;">    
                <?php 
					if(isset($dataCar[1093])){
						 if(is_array($dataCar[1093]) && count($dataCar[1093])>0){
							 foreach ($dataCar[1093] as $value) {
								 if(isset($value['v'])){
									  echo "<li><a href='javascript:;'>".$value['v']."</a></li>";
								 }
							 }
						 }
					}
...
/*#cen_id {height: 30px;} anh dung close*/
</style>